...
modules.get('apps/management')
  .directive('uploadWizard', function () {
    ...
    //   controller: function ($scope, AppState, safeConfirm, kbnUrl, Notifier, $window, Private) {
        //controller: function ($route, AppState, config, kbnUrl, Notifier, Private) {
        controller: function ($scope, AppState, confirmModal, kbnUrl, Notifier, $window, Private) {
        ...
        $scope.$watch('wizard.state.currentStep', (newValue, oldValue) => {
          ...
          if (newValue < oldValue) {
            ...
            confirmModal(
                `Going back will reset any changes you\'ve made to this step, do you want to continue?`,
                confirmModalOptions
            );

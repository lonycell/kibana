...
import {convertToSnakeCase, convertToCamelCase} from '../../../../common/lib/case_conversion';
...
export function registerPost(server) {
  ...
  server.route({
    ...
    handler: async function (req, reply) {
      ...
      const metaFields = await uiSettings.get(req,'metaFields');
      const { callWithRequest } = server.plugins.elasticsearch.getCluster('data');
      const boundCallWithRequest = _.partial(callWithRequest, req);
      const requestDocument = _.cloneDeep(req.payload);
      const indexPattern = convertToCamelCase(requestDocument.index_pattern);
